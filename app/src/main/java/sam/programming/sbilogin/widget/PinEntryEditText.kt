package sam.programming.sbilogin.widget
...
import sam.programming.sbilogin.R
...
class PinEntryEditText : androidx.appcompat.widget.AppCompatEditText {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val textWidths = textLength?.let { FloatArray(it) }
        if (textLength != null) {
        ...
        while (i < mNumChars) {
            ...
            if ((getText()?.length ?: 0) > i) {
                ...
                text?.let {
                    if (textWidths != null) {
                        canvas.drawText(
                            it, i, i + 1, middle - textWidths[0] / 2, bottom - mLineSpacing,
                            paint
                        )
                    }
                }
